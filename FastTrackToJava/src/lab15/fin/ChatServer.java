...
public class ChatServer implements Runnable, Closeable {
	...
	public void close() {
		if (selector != null && selector.isOpen()) {
			...
			try {
				...
				log("Failed to shut-down. Ignoring." + e);
			}
		}
	}
	...
	public void run() {
		try (ServerSocketChannel serverSocketChannel = ServerSocketChannel.open()) {
			...
			log("Chat server is ready on port " + daemonAddress);
			...
			while (selector.isOpen()) {
				...
				while (iterator.hasNext()) {
					...
					try {
						...
						log("Encountered an I/O error while processing I/O client requests. Ignoring" + e);
					}
				}
			}
		}
			...
			log("Caught error on close: " + exception);
		}
	}
